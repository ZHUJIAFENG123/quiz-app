<template>
  <div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
      ...
      <p class="text-sm text-gray-400">共 {{ total }} 道收藏题目</p>
    </div>
    ...
    <div v-else-if="list.length > 0" class="space-y-3">
      ...
      >
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0 cursor-pointer" @click="toggleExpand(item.id)">
            <div class="flex items-center gap-2 mb-1">
              ...
              <span v-if="item.chapter_name" class="text-xs text-gray-400">· {{ item.chapter_name }}</span>
            </div>
            <p class="text-sm text-gray-700 truncate">{{ item.content }}</p>
            <div class="flex items-center gap-3 mt-2 text-xs text-gray-400">
              <span v-if="item.favorited_at">
        ...
        <div v-if="expandedId === item.id" class="mt-3 pt-3 border-t border-gray-100">
          ...
          <div v-if="item.answer" class="text-sm mb-2">
            <span class="text-gray-500">正确答案：</span>
            <span class="text-green-600 font-medium">{{ item.answer }}</span>
          </div>
...
async function batchRemove() {
  ...
  if (!confirm(`确定要取消收藏选中的 ${selectedIds.value.length} 道题目吗？`)) return
  try {
    await API.batchRemoveFavorites(selectedIds.value)
