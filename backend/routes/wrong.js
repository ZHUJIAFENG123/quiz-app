  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      const answerRows = db.prepare(`
        ...
        WHERE question_id IN (${ids.map(() => '?').join(',')}) AND is_correct = 0
        ORDER BY id DESC
      `).all(...ids);
...
router.delete('/:questionId', (req, res) => {
  try {
    db.prepare('DELETE FROM wrong_questions WHERE question_id = ?').run(req.params.questionId);
...
router.post('/batch-remove', (req, res) => {
  try {
    ...
    db.prepare(`DELETE FROM wrong_questions WHERE question_id IN (${placeholders})`).run(...ids);
