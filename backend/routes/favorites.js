...
router.get('/', (req, res) => {
  try {
    ...
    const total = db.prepare('SELECT COUNT(*) as count FROM favorites WHERE user_id = ?').get(userId);
...
router.post('/:questionId', (req, res) => {
  try {
    ...
    const existing = db.prepare('SELECT * FROM favorites WHERE question_id = ? AND user_id = ?').get(req.params.questionId, userId);
    if (existing) return res.json({ success: true, message: '已收藏' });
    
    db.prepare('INSERT INTO favorites (question_id, user_id) VALUES (?, ?)').run(req.params.questionId, userId);
...
router.delete('/:questionId', (req, res) => {
  try {
    const userId = req.userId || 0;
    db.prepare('DELETE FROM favorites WHERE question_id = ? AND user_id = ?').run(req.params.questionId, userId);
...
router.post('/batch-remove', (req, res) => {
  try {
    ...
    db.prepare(`DELETE FROM favorites WHERE question_id IN (${placeholders}) AND user_id = ?`).run(...ids, userId);
